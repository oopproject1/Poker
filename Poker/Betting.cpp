...
int Betting::BettingRun(Player& player, Player& npc1, Player& npc2, Player& npc3) {
	...
	if (player.getLose() == true) {
		...
		while (round < 3) {
			...
			cin >> BettingType;
			if (BettingType == "1") {
				cout << "[레이즈]베팅금액을 입력하세요. 현재보유금액은 " << player.getMoney() << ", 판돈은 " << roundMoney << " 입니다." << endl;
				int tempBM;
				cin >> tempBM;
				raiseMoney += tempBM;
				...
				cout << "[체크] 체크하여 판돈을 추가하지 않습니다. 현재보유금액은 " << player.getMoney() << ", 판돈은 " << roundMoney << " 입니다. " << endl;
			}
			...
			if (BettingType == "1") {
				...
				cout << "[콜] 현재보유금액은 " << player.getMoney() << ", 판돈은 " << roundMoney << " 입니다. " << endl;
				player.setMoney(player.getMoney() - (n3round - pround));

				roundMoney += n3round - pround;
				cout << "현재보유금액은 " << player.getMoney() << ", 판돈은 " << roundMoney << " 입니다." << endl;
