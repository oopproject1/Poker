...
Card::Card(int code) {// test용 생성자
	shape_ = remainCard[code] / 13;
	value_ = (remainCard[code] % 13) + 1;
	if (shape_ == 0) {
		shape = 'C';
	}
	else if (shape_ == 1) {
...
void Card::cardprint() {//카드 프린트
	std::cout << shape << ", " << value << std::endl;
}
int Card::getShape() {//모양 받기
	return shape_;
}
int Card::getValue() {//숫자 받기
	return value_;
}
void Card::shuffle() {//카드 다시 섞기. 나중에 딜러 클래스로 옮겨야 함
	remain = 52;
	for (int i = 0; i < 52; i++) {
		remainCard[i] = i;
	}
